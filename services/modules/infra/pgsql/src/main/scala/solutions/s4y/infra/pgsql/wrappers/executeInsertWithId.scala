 ...
 * Usage: executeInsertWithId("INSERT INTO table (col1, col2) VALUES (?, ?)", _.setString(1, "value").setLong(2, 123L))
 */
def executeInsertWithId[T](
                            sql: String,
    ...
    ZIO.scoped {
      for {
        ...
        id <- ZIO
          ...
          .mapError(error => s"Failed to extract generated key: ${error.getMessage}")
      } yield id
    }
  }